...
Class Products_M extends CI_Model {
    ...
    public function get_all($filter_data=array(),$offset='',$limit=''){
        ...
        $this->db->select('a.*,(c.option_value_name) as option_name,IF(b.option_id IS NULL,a.quantity,b.option_qty) as stock_quantity,IF(b.option_id IS NULL,a.product_code,b.option_code) as product_option_code',false);
        ...
        if(!empty($filter_data)){
            ...
            if(!empty($filter_data['search'])){
                ...
                $search='(a.product_name LIKE "%'.$search.'%"" OR b.option_code LIKE "%'.$search.'%")';
                $this->db->where($search);
            }
            if(!empty($filter_data['product_name'])){
                $search = $filter_data['product_name'];
                $search='(a.product_name LIKE "%'.$search.'%" OR b.option_name LIKE "%'.$search.'%")';
                $this->db->where($search);
            }
    ...
    public function get_total($filter_data){
        ...
        if(!empty($filter_data)){
            ...
            if(isset($filter_data['search'])){
                $search = $filter_data['search'];
                $search='(a.product_name LIKE "%'.$search.'%")';
                $this->db->where($search);
            }
            if(!empty($filter_data['product_name'])){
                $search = $filter_data['product_name'];
	...
	public function add($postdetails) {
		...
		// $option['short_description']=$postdetails['short_description'];
		// $option['description']=$postdetails['description'];
